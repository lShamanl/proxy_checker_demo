...
namespace App\ProxyChecker\Entry\Http\Admin\Api\Proxy\Read;
...
class Action
{
     ...
     * @OA\Response(
     *     response=200,
     *     description="Read query for Proxy",
     *     @OA\JsonContent(
     *         allOf={
     ...
     */
    #[Route(
        path: '/api/admin/proxy-checker/proxies/{id}.{_format}',
        name: self::NAME,
    ...
    public function action(
        ...
        /** @var Proxy $proxy */
        $proxy = $bus->query($query);

        return $presenter->present(
            data: ApiFormatter::prepare(
                CommonOutputContract::create($proxy)
            ),
            outputFormat: $outputFormat
        );
    }
